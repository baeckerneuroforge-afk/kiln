    ...
    <div className="flex flex-col h-full min-h-[calc(100vh-64px)]">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-border">
        <div className="flex items-center gap-4">
          <Link href="/dashboard/teams" className="text-zinc-500 hover:text-zinc-300">
            <ArrowLeft className="h-5 w-5" />
          </Link>
          <h1 className="text-xl font-semibold text-zinc-100 flex items-center gap-2 font-[family-name:var(--font-instrument)]">
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      selectedTestId === test.id && "bg-zinc-900/80 border-l-2 border-orange-500"
                    )}
                  >
                    <div className="flex items-center justify-between mb-1.5">
                      <span className="font-medium text-sm text-zinc-200 truncate">{test.name}</span>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] text-zinc-500 min-w-[50px] text-right">
                      ...
                      {test.status === "RUNNING" && (
                        <>
                          ...
                          <button
                            ...
                            className="text-[10px] px-2 py-0.5 rounded-md bg-zinc-800 text-zinc-400 hover:bg-zinc-700"
                          >
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    {detailTest.status === "RUNNING" && (
                      <>
                        <Button size="sm" variant="outline" onClick={() => updateStatus(detailTest.id, "PAUSED")} className="text-zinc-400">
                          <Pause className="h-3.5 w-3.5 mr-1" /> Pause
                        </Button>
                        <Button size="sm" variant="outline" onClick={() => updateStatus(detailTest.id, "COMPLETED")} className="text-zinc-400">
                          <Check className="h-3.5 w-3.5 mr-1" /> Complete
                        </Button>
                      </>
                    )}
                    {detailTest.status === "PAUSED" && (
                      <Button size="sm" variant="outline" onClick={() => updateStatus(detailTest.id, "RUNNING")} className="text-zinc-400">
                        <Play className="h-3.5 w-3.5 mr-1" /> Resume
                      </Button>
                    )}
                    <button onClick={() => { setSelectedTestId(null); setDetailTest(null); }} className="text-zinc-500 hover:text-zinc-300 p-1">
                      <X className="h-4 w-4" />
                    </button>
                  </div>
                </div>
                ...
                {detailTest.stats && detailTest.stats.winner !== "INSUFFICIENT_DATA" && (
                  <div className={cn(
                    ...
                    <Trophy className={cn("h-5 w-5", detailTest.stats.winner === "TIE" ? "text-zinc-400" : "text-orange-400")} />
                    <div>
                      ...
                      return (
                        <div
                          ...
                          className={cn(
                            ...
                            isWinner ? "border-orange-500/40 bg-orange-500/5" : "border-zinc-800 bg-zinc-900/60"
                          )}
                        >
                          <div className="flex items-center justify-between">
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            <div className="rounded-lg bg-zinc-950/50 p-2.5">
                            ...
                            <div className="rounded-lg bg-zinc-950/50 p-2.5">
                              <div className="text-[10px] text-zinc-500 mb-1">Executions</div>
                              <div className="text-lg font-bold text-zinc-100">{stats.executions}</div>
                            </div>
                            <div className="rounded-lg bg-zinc-950/50 p-2.5">
                              <div className="text-[10px] text-zinc-500 mb-1">Avg Cost</div>
                              <div className="text-lg font-bold text-zinc-100">
                                {formatCost(stats.avgCost)}
                              </div>
                            </div>
                            <div className="rounded-lg bg-zinc-950/50 p-2.5">
                              <div className="text-[10px] text-zinc-500 mb-1">Avg Duration</div>
                              <div className="text-lg font-bold text-zinc-100">
                          ...
                          <div className="pt-2 border-t border-zinc-800/50">
                            ...
                            <div className="flex items-center justify-between text-[11px] mt-1">
                              <span className="text-zinc-500">Completed / Failed</span>
                              <span className="text-zinc-300">{stats.completed} / {stats.failed}</span>
                            </div>
                          </div>
                        </div>
                      );
                ...
                {detailTest.stats && detailTest.stats.teamA.executions > 0 && (
                  <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-4 space-y-4">
                    <h3 className="text-sm font-medium text-zinc-300">Comparison</h3>
                ...
                {detailTest.results && detailTest.results.length > 0 && (
                  <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-4">
                    <h3 className="text-sm font-medium text-zinc-300 mb-3">Recent Executions</h3>
                    <div className="space-y-1.5">
                      {detailTest.results.slice(0, 20).map((result) => (
                        <div key={result.id} className="flex items-center gap-3 text-[11px] px-2 py-1.5 rounded-lg hover:bg-zinc-800/50">
                          ...
                          {result.cost != null && (
                            <span className="text-zinc-500">{formatCost(result.cost)}</span>
                          )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 w-full max-w-lg shadow-2xl">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  />
                  <span className="text-sm text-zinc-300 min-w-[60px] text-right font-mono">
                    {split}/{100 - split}
                  </span>
                </div>
              </div>
              ...
              <div>
                <label className="text-xs text-zinc-500 mb-1 block">Success Metric</label>
            ...
            <div className="flex justify-end gap-3 mt-6">
              <Button variant="ghost" size="sm" onClick={() => setShowCreate(false)} className="text-zinc-400">
                Cancel
              </Button>
              <Button
                size="sm"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => { setShowExecute(null); setExecuteGoal(""); }} className="text-zinc-500 hover:text-zinc-300">
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <textarea
              ...
              rows={3}
              className="w-full bg-zinc-800/60 border border-zinc-700 rounded-xl text-sm text-zinc-200 outline-none p-3 placeholder:text-zinc-600 resize-none focus:border-orange-500/50"
            />
            <div className="flex justify-end gap-3 mt-4">
              <Button variant="ghost" size="sm" onClick={() => { setShowExecute(null); setExecuteGoal(""); }} className="text-zinc-400">
                Cancel
              </Button>
              <Button
                size="sm"
