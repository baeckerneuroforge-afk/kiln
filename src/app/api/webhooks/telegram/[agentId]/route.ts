...
export async function POST(request: NextRequest, { params }: { params: Promise<{ agentId: string }> }) {
  try {
    ...
    if (!creditCheck.allowed) {
      console.warn(`Telegram webhook: insufficient credits for agent ${agentId}, user ${agent.userId}`);
      return Response.json({ ok: true }); // Return 200 to prevent Telegram retries
    }
