...
export function registerTeamTools(server, client) {
    // ── kiln_list_teams ──
    server.tool("kiln_list_teams", "List all Agent Teams. Returns id, name, goal, status, member count, and task count.", {}, async () => client.callTool("kiln_list_teams", {}));
    // ── kiln_create_team ──
    server.tool("kiln_create_team", "Create a new Agent Team. Optionally use a template (SALES, SUPPORT, CONTENT) to auto-generate team structure.", {
    ...
    server.tool("kiln_add_team_member", "Add an agent to a team with a specific role (HEAD, COORDINATOR, EXECUTOR, REPORTER).", {
    ...
    // ── kiln_assign_task ──
    server.tool("kiln_assign_task", "Assign a task to the team's HEAD agent for delegation. The HEAD will decompose it into subtasks.", {
        teamId: z.string().describe("Team ID"),
    ...
    // ── kiln_execute_team ──
    server.tool("kiln_execute_team", "Execute a team workflow: decompose a goal into subtasks assigned to team members using the HEAD agent. Returns all generated tasks.", {
        teamId: z.string().describe("Team ID"),
    ...
    server.tool("kiln_get_team_status", "Get current status of a team: members, tasks, progress.", {
        teamId: z.string().describe("Team ID"),
    }, async (args) => client.callTool("kiln_get_team_status", args));
    // ── kiln_orchestrate ──
    server.tool("kiln_orchestrate", "Define an agent-to-agent handoff rule. When the condition matches in the source agent's conversation, it triggers a handoff to the target agent.", {
    ...
    server.tool("kiln_create_automation", "Create a scheduled automation rule for an agent. The agent will execute the task on the given schedule.", {
    ...
    server.tool("kiln_create_workflow_automation", "Create a scheduled or webhook-triggered automation with cron expressions, input templates, and notification routing.", {
    ...
    // ── kiln_list_workflows ──
    server.tool("kiln_list_workflows", "List all automations, team configurations, and orchestration rules. Provides a complete overview of all workflows.", {}, async () => client.callTool("kiln_list_workflows", {}));
}
