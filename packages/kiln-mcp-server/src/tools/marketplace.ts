...
export function registerMarketplaceTools(server: McpServer, client: KilnClient): void {
  // ── kiln_get_analytics ──
  server.tool(
    "kiln_get_analytics",
  ...
  server.tool(
    "kiln_list_webhooks",
    "List all inbound webhooks for an agent with their URLs, auth config, and execution stats.",
    {
      agentId: z.string().describe("Agent ID"),
    },
