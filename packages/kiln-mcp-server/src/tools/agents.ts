...
export function registerAgentTools(server: McpServer, client: KilnClient): void {
  // ── kiln_list_agents ──
  server.tool(
    "kiln_list_agents",
    ...
    {},
    async () => client.callTool("kiln_list_agents", {})
  );

  // ── kiln_create_agent ──
  server.tool(
    "kiln_create_agent",
    "Create a new AI agent. Set agentMode to CHAT for conversational agents or TASK for autonomous background agents.",
    {
  ...
  server.tool(
    "kiln_export_agent_config",
    "Export an agent's full configuration as JSON. Includes all settings, actions, prompt branches, custom tools, and task config. Does NOT include conversations, knowledge base content, or API keys.",
    {
