...
export function registerChatTools(server: McpServer, client: KilnClient): void {
  // ── kiln_chat ──
  server.tool(
    "kiln_chat",
    "Send a message to a Chat Agent and get a response. Use sessionId to maintain conversation context across messages.",
    {
  ...
  server.tool(
    "kiln_run_agent",
    "Manually trigger a Task Agent with text input and return the execution result. Only works for agents with agentMode=TASK.",
    {
  ...
  server.tool(
    "kiln_run_task",
    "Execute a Task Agent with structured JSON input. Supports full tool calling, pre/post-processing, and output routing. Waits for completion.",
    {
      agentId: z.string().describe("Agent ID of the task agent to run"),
