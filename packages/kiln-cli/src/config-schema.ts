...
type KilnConfigInput = Omit<KilnConfig, "model" | "knowledge" | "actions"> & {
  model?: string;
  knowledge?: {
    urls?: string[];
    files?: string[];
...
function expectString(value: unknown, field: string, required = true): string | undefined {
...
export function normalizeConfig(input: unknown): KilnConfig {
  if (!input || typeof input !== "object") {
    throw new Error("Invalid config: YAML root must be an object.");
  }

  const data = input as KilnConfigInput;
  ...
  if (data.embed) {
    ...
    embed = {
      type: embedType as "bubble" | "iframe",
